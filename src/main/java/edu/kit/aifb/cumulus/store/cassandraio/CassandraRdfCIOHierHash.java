...
public class CassandraRdfCIOHierHash extends CassandraRdfCIOQuads {
	
	static final String CF_SPO = "SPO";
	static final String CF_POS = "POS";
	static final String CF_OSP = "OSP";
	
	transient private final Logger _log = Logger.getLogger(this.getClass().getName()); //CassandraRdfCIOHierHash.class);
	
	public CassandraRdfCIOHierHash(String hosts) {
		this(hosts, DEFAULT_KS);
	}
	
	public CassandraRdfCIOHierHash(String hosts, String keyspace) {
	...
	public Iterator<Node[]> query(Node[] query, int limit) throws StoreException {
		...
		// based on the query, find out which column family to use
		String columnFamily = selectColumnFamily(query);
		int[] map = _maps.get(columnFamily);	
		// re-order (s,p,o) based on the column family to be used for querying

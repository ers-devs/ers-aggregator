                ...
                if( crdf.keyspaceEnabledVersioning(a) ) {
                    if( urn == null || urn.isEmpty() ) {
                        sendResponse(ctx, req, resp, HttpServletResponse.SC_BAD_REQUEST,
                                "Please pass 'URN' as " + a + " is versioned. ");
                        return;
                    }
                    update_result = crdf.updateDataVersioning(e,p,v_old,v_new,
                            Store.encodeKeyspace(a), 0, urn);
                }
                else {
                    update_result = crdf.updateData(e,p,v_old,v_new,
                            Store.encodeKeyspace(a), 0);
                }
